...
* Download <a href="./data/10-s02/L3_stars.csv" target = "_blank">L3_stars.csv</a> and <a href="./data/10-s02/psess.csv" target = "_blank">psess.csv</a> and save them in a folder. Make sure that you do not change the file name at all.
...
<img src="10-s02-lab10a_files/figure-html/scatter-1.png" alt="Scatteplot of mean anxiety and attendance" width="100%" />
...
2. To three decimal places, if the GLM for this model is $Y_i = b_0 + b_1 X_i + e_i$, then $b_1$ is <input class='webex-solveme nospaces' size='6' data-answer='["-2.173"]'/>
...
If you do not have these packages installed on your machine yet, you can go ahead and install them by typing `install.packages("NAME OF PACKAGE")` in the console below. If you are working on the server, the packages are already installed.
...
<div class="figure" style="text-align: center">
<img src="10-s02-lab10a_files/figure-html/check_model-1.png" alt="Visual assumption checks" width="100%" />
...
<img src="10-s02-lab10a_files/figure-html/unnamed-chunk-2-1.png" width="100%" style="display: block; margin: auto;" />
...
<img src="10-s02-lab10a_files/figure-html/unnamed-chunk-3-1.png" width="100%" style="display: block; margin: auto;" />
...
<img src="10-s02-lab10a_files/figure-html/unnamed-chunk-4-1.png" width="100%" style="display: block; margin: auto;" />
...
There's two ways we can use R to help with the write-up. The first is inline coding like we've done previously, and the second is to use the `report` package. Which one you use is entirely up to you but it's nice to have options.
...
A simple linear regression was performed with engagement (M = 4.54, SD = 0.56) as the outcome variable and statistics anxiety (M = 2.08, SD = 0.56) as the predictor variable. The results of the regression indicated that the model significantly predicted course engagement (F(1, 35) = 11.99, p < .001, Adjusted R2 = 0.23, f^2^ = .63), accounting for 23% of the variance. Anxiety was a significant predictor (b = -2.17, p < 0.001.
)

The second option uses `report`. The output of these functions doesn't tend to be usable without some editing but particularly when you're first learning how to write-up stats it can be useful to have this kind of template (and also to see that there's different ways of reporting stats).
