...
library("performance")
library("tidyverse")
dat <- read_csv("data/12-s02/James_Holmes_Expt 2_DATA.csv")%>%
...
```{block, type="info"} 
There are quite a few effect sizes around that are used in research. We learned about Cohen's D in a previous chapter and here we are using partial eta-squared. The [Psychometrica resource](https://www.psychometrica.de/effect_size.html) can help transforming one effect size into another. It also provides a nice table (under section 16) that helps with interpreting different effect sizes. Take a look!
```
