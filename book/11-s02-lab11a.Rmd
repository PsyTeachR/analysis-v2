...
```{r ch11-load-hide, message = FALSE, warning = FALSE, echo = FALSE}
library("pwr")
library("see")
library("performance")
library("report")
library("tidyverse")
pinfo <- read_csv("data/11-s02/participant_info.csv")
wellbeing <- read_csv("data/11-s02/wellbeing.csv")
screen <- read_csv("data/11-s02/screen_time.csv")
```


```{r load, message = FALSE, echo = FALSE, eval=FALSE}
library("pwr")
library("see")
library("performance")
library("report")
...
* Using the code for the power analysis calculate the minimum effect size we could reliably observe given our sample size and design but for 99% power. Report this to 2 decimal places `r fitb(".00", "0.00")`
...
```{r hidden-a11a, eval = FALSE, echo=FALSE}
newdata <- tibble(male_c = "-0.5", thours_c = 4)
```

```{r hidden-a11b, eval = FALSE, echo=FALSE}
predict(mod, newdata)
```

Now, based on the output try to answer this question: 

* To one decimal place, what is the predicted well-being rating of a girl with smartphone use of 4 hours - `r fitb(41.8, width = 4)`
...
> All continuous predictors were mean-centered and deviation coding was used for categorical predictors. The results of the regression indicated that the model significantly predicted course engagement (F(`r mod_summary$fstatistic[2]`, `r mod_summary$fstatistic[3]`) = `r mod_summary$fstatistic[1] %>% round(2)`, p < .001, Adjusted R2 = `r mod_summary$adj.r.squared %>% round(2)`, f2 = .63), accounting for `r (mod_summary$adj.r.squared %>% round(2))*100`% of the variance. Total screen time was a significant negative predictor of well-being scores (b = `r mod$coefficients[2] %>% round(2)`, p < .001, as was gender (b = `r mod$coefficients[3] %>% round(2)`, p < .001, with girls having lower well-being scores than boys. Importantly, there was a significant interaction between screen time and gender (b = `r mod$coefficients[4] %>% round(2)`, p < .001), smartphone use was more negatively associated with well-being for girls than for boys.
