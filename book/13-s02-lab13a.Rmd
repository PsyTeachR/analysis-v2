...
factorial <- read_csv("data/13-s02/Zhang et al. 2014 Exp 3.csv")%>%
...
```{r fact_dat, message=FALSE, eval=FALSE}
...
`r hide()`
```{r t1A_sol, message = FALSE, warning=FALSE, eval=FALSE}
library(afex)
library(tidyverse)
```
`r unhide()`
...
```{r t2-hide_sol, echo = FALSE, message = FALSE, warning=FALSE}
knitr::kable(cell_means)
```
...
```{r t3-hide_sol, echo = FALSE, message = FALSE}
knitr::kable(marg_sex)
```
...
```{r t4-hide_sol, echo = FALSE, message = FALSE}
knitr::kable(marg_cond)
```
...
`r hide()`
```{r t5_sol, fig.cap='You should have produced a similar figure'}
ggplot(cell_means, aes(condition, m, shape = sex, group = sex, color = sex)) +
  geom_line() +
  geom_point(size = 3) +
  labs(y = "mean looking time (msecs)") +
...
`r hide()`
```{r t9_sol, warning=FALSE, message=FALSE}
mod <- aov_ez(data = dat,
                   dv = "looktime", 
                   id = "subject",
                   type = 3,
