...
The main statistic (the test-value) of the Mann-Whitney U-test is called the U-value and is shown in the above table as `statistic`; i.e. U = `r result %>% pull(statistic)` and you can see from the results that the difference was non-significant as p = `r result %>% pull(p.value) %>% round(3)`. 
...
The U-value for these two tests are, for Version 1, U = `r result_v1$statistic` and for Version 2, U = `r result_v2$statistic`. This may seem odd, but actually both those test are correct. However, strictly speaking the U-value is the smaller of the two-values given by the different outputs. It is to do with how the U-value is calculated. Both groups have a U-value and the one that is checked for significance is the smaller of the two.
